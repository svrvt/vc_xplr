--author: https://github.com/sayanarijit/tree-view.xplr
--license: MIT https://github.com/sayanarijit/tree-view.xplr/blob/main/LICENSE
...
local does_support_deep_branch_navigation = xplrv and xplrv.major >= 0 and xplrv.minor >= 21 and xplrv.patch >= 4
...
local function explore(path, explorer_config)
	...
	local branch = new_branch(path, nodes, explorer_config, old_branch and old_branch.all_expanded)
...
local function render_node(node)
	...
	if node.is_symlink then
		...
		if node.is_broken then
			...
			local symlink_path = xplr.util.shorten(node.symlink.absolute_path, { base = node.parent })
...
local function render(ctx)
	...
	if state.pwd ~= state.root and string.sub(state.pwd, 1, #state.root + 1) ~= state.root .. "/" then
...
local function open(app)
	...
	elseif fallback then
		err = "# of nodes in this branch is more than fallback threshold: " .. tostring(state.fallback_threshold)
	end
...
local function setup(args)
	...
	if args.as_default_layout == true then
		if state.fullscreen then
			xplr.config.layouts.builtin.default = xplr.config.layouts.custom.tree_view_fullscreen
		else
